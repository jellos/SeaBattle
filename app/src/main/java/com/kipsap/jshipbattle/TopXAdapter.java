...
public class TopXAdapter extends ArrayAdapter {

	// used to keep selected position in ListView
	private int selectedPos = -1;	// init value for not-selected
	private List<?> realRanks;
	private List<?> names;
	...
	public TopXAdapter(Context context, int textViewResourceId, List<?> theRealRanks,
			List<?> theNames, List<?> theRatings, List<?> theCountrys)
	{		
		super(context, textViewResourceId, theNames);
		_context = context;
		realRanks = theRealRanks;
		names = theNames;
		ratings = theRatings;
	...
	public View getView(int position, View convertView, ViewGroup parent) 
	{
        ...
        rank.setText("" + realRanks.get(position).toString());
